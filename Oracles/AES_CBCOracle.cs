...
{
	public class AES_CBCOracle : ICBCOracle
	{
		private byte[] key;
		...
		public bool RequestOracle(byte[] cipher)
        {
            ...
            byte[] decrypted = AES_DecryptBlock(cipherData, this.key);
            decrypted = Helpers.Xor(decrypted, cipherIV);
		...
		public string AES_DecryptToString(byte[] cipherText)
        {
            if (cipherText == null || cipherText.Length < 16)
		...
		public byte[] AES_EncryptString(string plainText)
        {
